//5th update, adding leave function without warning.
//Should work for if 1 node leaves or if up to all the network to the end leaves.
...
int sendPacket(struct packetType sPacket){
  ...
  uint8_t diffad = sendtoad[0] - selfad[0];
  ...
  while(a==0){
    ...
    if (a==0){
      sendtoad[0] += diffad; //try to send to next node
    }
  }
  if (abs(sendtoad[0] - selfad[0]) > 1){
    selfadChange = sendtoad[0] - selfad[0];
    NWcounter = NWcounter - selfadChange;
  }
  //if sending to left, have to change selfad too.
  if (sendtoad[0] - selfad[0] < -1) selfad[0] = selfad[0] - selfadChange;
  updateNodes(1, selfadChange);
  updateNodes(NWcounter, selfadChange);
  
  return a;
}
//-------------------------------
//Used to inform every node about the new NWcounter
int updateNodes(uint8_t recNode, uint8_t selfadChange){
  ...
  updatePacket.data[1] = selfadChange;
...
void loop() {//in the network
  ...
  if(packet.dataSize!=0)//have data
  {
    ...
    if(strcmp(packet.type, "update")==0)
    {
      ...
      selfad[0] = selfad[0] + packet.data[1];
